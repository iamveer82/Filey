      ...
      <View style={[styles.txnCard, darkMode ? CardPresets.cardDark : CardPresets.cardLight]}>
        {isEditing ? (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
              {CATEGORIES.slice(1).map(cat => (
                <TouchableOpacity
                  ...
                  style={[styles.catChip, { backgroundColor: editCat === cat ? '#0F53DC' : c.surfaceLow, borderColor: editCat === cat ? '#0A3DA6' : c.border }]}
                >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md }}>
            <View style={[styles.txnIcon, { backgroundColor: 'rgba(15,83,220,0.12)' }]}>
              <Ionicons name={icon} size={20} color="#0F53DC" />
            </View>
            <View style={{ flex: 1 }}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              ...
              {parseFloat(file.vat) > 0 && (
                <Text style={[styles.txnVat, { color: '#0F53DC' }]}>VAT {parseFloat(file.vat).toFixed(2)}</Text>
              )}
            </View>
            <TouchableOpacity
    ...
    <View style={[styles.root, { backgroundColor: c.bg }]}>
      ...
      <View style={styles.tabRow}>
        {[{ id: 'ledger', label: 'Ledger' }, { id: 'vault', label: 'FTA Vault' }].map(tab => (
          <TouchableOpacity
            ...
            style={[styles.tab, { backgroundColor: activeTab === tab.id ? '#0F53DC' : c.surfaceLow, borderColor: activeTab === tab.id ? '#0A3DA6' : c.border }]}
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={{ paddingHorizontal: Spacing.xxl, gap: Spacing.sm }}>
        {CATEGORIES.map(cat => (
          <TouchableOpacity
            ...
            style={[styles.filterChip, { backgroundColor: filter === cat ? '#0F53DC' : c.surfaceLow, borderColor: filter === cat ? '#2563EB' : c.border }]}
            accessibilityRole="button"
      ...
      <View style={[styles.summaryStrip, { backgroundColor: c.surfaceLow, borderColor: c.border }]}>
        ...
        <View style={styles.summaryItem}>
          ...
          <Text style={[styles.summaryValue, { color: '#0F53DC' }]}>{totalVat.toLocaleString('en-AE', { minimumFractionDigits: 2 })} AED</Text>
        </View>
        <View style={[styles.summaryDivider, { backgroundColor: c.border }]} />
        <View style={styles.summaryItem}>
      ...
      {loading ? (
        <View style={styles.loadingWrap}>
          <ActivityIndicator size="large" color="#0F53DC" />
        </View>
      ) : filtered.length === 0 ? (
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.exportModal, darkMode ? CardPresets.cardElevatedDark || CardPresets.cardDark : CardPresets.cardLight]}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {exportData && (
                <>
                  <View style={[styles.tableHeader, { backgroundColor: '#0F53DC' }]}>
                  ...
                  {(exportData.transactions || []).map((t, i) => (
                    <View key={i} style={[styles.tableRow, { borderColor: c.border }]}>
                      ...
                      <Text style={[styles.tdText, { flex: 0.8, color: '#0F53DC' }]}>{(t.vat || 0).toFixed(1)}</Text>
                  ...
                  <View style={[styles.totalSection, { borderColor: c.border }]}>
                    {[
                      ...
                      { label: 'VAT 5%', value: exportData.totalVat, color: '#0F53DC' },
...
const styles = StyleSheet.create({
  ...
  exportBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#0F53DC', paddingHorizontal: Spacing.lg, paddingVertical: 10, borderRadius: Radius.pill, borderWidth: 1, borderColor: 'rgba(15,83,220,0.35)' },
  ...
  trnBadge: { backgroundColor: 'rgba(15,83,220,0.15)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  trnBadgeText: { fontSize: 9, fontWeight: '800', color: '#0F53DC', letterSpacing: 0.5 },
  ...
  confirmBtn: { flex: 1, backgroundColor: '#0F53DC', borderRadius: Radius.md, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderWidth: 1, borderColor: 'rgba(15,83,220,0.35)' },
