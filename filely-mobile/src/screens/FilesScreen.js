      ...
      <View style={[styles.txnCard, darkMode ? CardPresets.cardDark : CardPresets.cardLight]}>
        {isEditing ? (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
              {CATEGORIES.slice(1).map(cat => (
                <TouchableOpacity
                  ...
                  style={[styles.catChip, { backgroundColor: editCat === cat ? '#44e571' : c.surfaceLow, borderColor: editCat === cat ? '#00531f' : c.border }]}
                >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md }}>
            <View style={[styles.txnIcon, { backgroundColor: 'rgba(79,142,255,0.12)' }]}>
              <Ionicons name={icon} size={20} color="#4F8EFF" />
            </View>
            <View style={{ flex: 1 }}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              ...
              {parseFloat(file.vat) > 0 && (
                <Text style={[styles.txnVat, { color: '#44e571' }]}>VAT {parseFloat(file.vat).toFixed(2)}</Text>
              )}
            </View>
            <TouchableOpacity
    ...
    <View style={[styles.root, { backgroundColor: c.bg }]}>
      ...
      <View style={styles.tabRow}>
        {[{ id: 'ledger', label: 'Ledger' }, { id: 'vault', label: 'FTA Vault' }].map(tab => (
          <TouchableOpacity
            ...
            style={[styles.tab, { backgroundColor: activeTab === tab.id ? '#44e571' : c.surfaceLow, borderColor: activeTab === tab.id ? '#006e2c' : c.border }]}
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={{ paddingHorizontal: Spacing.xxl, gap: Spacing.sm }}>
        {CATEGORIES.map(cat => (
          <TouchableOpacity
            ...
            style={[styles.filterChip, { backgroundColor: filter === cat ? '#4F8EFF' : c.surfaceLow, borderColor: filter === cat ? '#2563EB' : c.border }]}
            accessibilityRole="button"
      ...
      <View style={[styles.summaryStrip, { backgroundColor: c.surfaceLow, borderColor: c.border }]}>
        ...
        <View style={styles.summaryItem}>
          ...
          <Text style={[styles.summaryValue, { color: '#44e571' }]}>{totalVat.toLocaleString('en-AE', { minimumFractionDigits: 2 })} AED</Text>
        </View>
        <View style={[styles.summaryDivider, { backgroundColor: c.border }]} />
        <View style={styles.summaryItem}>
      ...
      {loading ? (
        <View style={styles.loadingWrap}>
          <ActivityIndicator size="large" color="#4F8EFF" />
        </View>
      ) : filtered.length === 0 ? (
                ...
                <>
                  ...
                  {(exportData.transactions || []).map((t, i) => (
                    <View key={i} style={[styles.tableRow, { borderColor: c.border }]}>
                      ...
                      <Text style={[styles.tdText, { flex: 0.8, color: '#44e571' }]}>{(t.vat || 0).toFixed(1)}</Text>
                  ...
                  <View style={[styles.totalSection, { borderColor: c.border }]}>
                    {[
                      ...
                      { label: 'VAT 5%', value: exportData.totalVat, color: '#44e571' },
...
const styles = StyleSheet.create({
  ...
  trnBadge: { backgroundColor: 'rgba(79,142,255,0.15)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  trnBadgeText: { fontSize: 9, fontWeight: '800', color: '#4F8EFF', letterSpacing: 0.5 },
  ...
  confirmBtn: { flex: 1, backgroundColor: '#44e571', borderRadius: Radius.md, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderWidth: 1, borderColor: 'rgba(0,83,31,0.3)' },
