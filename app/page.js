...
export default function App() {
  const [screen, setScreen] = useState('loading'); // loading | onboarding | auth | app
  ...
  useEffect(() => {
    ...
    else if (onboarded) { setScreen('auth'); }
    else { setTimeout(() => setScreen('onboarding'), 800); }
  }, []);

  const handleOnboarded = () => { localStorage.setItem('filey_onboarded', '1'); setScreen('auth'); };
  const handleAuth = (user) => { setProfile(user); setScreen('app'); };
  const handleLogout = () => { localStorage.removeItem('filey_token'); localStorage.removeItem('filey_user'); setScreen('auth'); setProfile(null); setShowSettings(false); };
  ...
  const sendMessage = async () => {
    ...
    try { const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,sessionId})}); const d=await r.json();
      if(d.error) setChatMessages(p=>[...p,{id:uuidv4(),role:'assistant',content:`Error: ${d.error}`,timestamp:new Date().toISOString()}]);
      else { setChatMessages(p=>[...p,{id:uuidv4(),role:'assistant',content:d.message,extractedTransaction:d.extractedTransaction,timestamp:d.timestamp}]); if(d.extractedTransaction) setPendingTransaction(d.extractedTransaction); if(d.sessionId) setSessionId(d.sessionId); }
    } catch { setChatMessages(p=>[...p,{id:uuidv4(),role:'assistant',content:'Connection error. Working offline.',timestamp:new Date().toISOString()}]); }
  ...
  return (
    <div className="min-h-[100dvh] bg-[#F7F8FA] font-['Plus_Jakarta_Sans'] relative overflow-x-hidden no-bounce">
      ...
      <AnimatePresence>{showSettings && <SettingsPanel profile={profile} setProfile={setProfile} onClose={()=>setShowSettings(false)} onLogout={handleLogout} />}</AnimatePresence>
...
// ═══════════════════════════════════════════════════════════════════
function SettingsPanel({ profile, setProfile, onClose, onLogout }) {
