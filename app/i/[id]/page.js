  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      if (totals) {
        ...
        draw(fmtCur(cur, totals.sub), 510, 500, { size: 10 });
        draw('VAT', 400, 482, { size: 10, color: slate });
        draw(fmtCur(cur, totals.vat), 510, 482, { size: 10 });
        draw('TOTAL', 400, 460, { size: 11, bold: true });
        draw(fmtCur(cur, totals.total), 510, 460, { size: 12, bold: true, color: brand });
        if (cur !== 'AED' && +invoice.fxRate > 0) {
          draw(`AED equivalent (FX ${(+invoice.fxRate).toFixed(4)}):`, 40, 460, { size: 8, color: slate });
          draw(`${fmtCur('AED', totals.total * (+invoice.fxRate))} total`, 40, 448, { size: 9, bold: true });
        }
      }
      ...
      >
        ...
        <header className="px-8 py-6 text-white" style={{ background: `linear-gradient(135deg, ${BRAND}, ${BRAND_DARK})` }}>
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right">
              ...
              <div className="text-2xl font-bold">{fmtCur(invoice.currency || 'AED', totals.total)}</div>
              {invoice.currency && invoice.currency !== 'AED' && +invoice.fxRate > 0 && (
                <div className="mt-0.5 text-[11px] opacity-80">≈ {fmtCur('AED', totals.total * (+invoice.fxRate))}</div>
              )}
            </div>
          </div>
        </header>
        ...
        <div className="border-t border-slate-100 px-8 py-5">
          <div className="ml-auto max-w-xs space-y-1 text-sm">
            <Row label="Subtotal" value={fmtCur(invoice.currency || 'AED', totals.sub)} />
            <Row label={`VAT${invoice.vatRate ? ` (${invoice.vatRate}%)` : ''}`} value={fmtCur(invoice.currency || 'AED', totals.vat)} />
            <div className="mt-2 flex items-center justify-between border-t border-slate-200 pt-2">
              <span className="text-sm font-bold" style={{ color: INK }}>Total</span>
              <span className="text-base font-bold" style={{ color: BRAND }}>{fmtCur(invoice.currency || 'AED', totals.total)}</span>
            </div>
            {invoice.currency && invoice.currency !== 'AED' && +invoice.fxRate > 0 && (
              <div className="mt-1 flex items-center justify-between text-[11px] text-slate-500">
                <span>≈ AED equiv. (FX {(+invoice.fxRate).toFixed(4)})</span>
                <span className="font-semibold" style={{ color: INK }}>{fmtCur('AED', totals.total * (+invoice.fxRate))}</span>
              </div>
            )}
          </div>
        </div>
